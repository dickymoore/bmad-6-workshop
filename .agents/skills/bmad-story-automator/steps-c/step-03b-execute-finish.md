...
- If `is_done == true` → proceed to F

### F. Finalize State Before Commit

Display: "**✅ Story {N} ready to commit.**"

```bash
"{scriptsDir}" orchestrator-helper state-update "{outputFile}" \
  --set lastUpdated="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "- **[$(date -u +%Y-%m-%dT%H:%M:%SZ)]** Story {story_id}: final state sealed before commit (sprint-status verified)" >> "{outputFile}"

# Update Story Progress before commit so the story commit captures complete finalization state.
tmp_state=$(mktemp)
sed "s/^| ${story_id} |.*$/| ${story_id} | done | done | done | done | done | done |/" "{outputFile}" > "$tmp_state" && mv "$tmp_state" "{outputFile}"
```

### G. Git Commit and State Cleanliness Check
...
- If `ok == true`:
  ```bash
  state_status=$(git -C "{project-root}" status --porcelain -- "{outputFile}" || true)
  if [ -n "$state_status" ]; then
      echo "$state_status"
  fi
  ```
  - If `state_status` is not empty → display "**Post-commit orchestration state is dirty; stop before continuing to the next story.**" and escalate.
  - If `state_status` is empty → proceed to H.
