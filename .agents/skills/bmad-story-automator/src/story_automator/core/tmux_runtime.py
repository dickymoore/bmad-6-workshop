...
def cleanup_runtime_artifacts(session: str, project_root: str | None = None, *, preserve_output: bool = False) -> None:
    paths = session_paths(session, project_root)
    for path in (paths.state, paths.command, paths.runner):
...
def tmux_kill_session(session: str, project_root: str | None = None) -> None:
    ...
    _export_output_file(session, root)
    if command_exists("tmux"):
        run_cmd("tmux", "kill-session", "-t", session)
    cleanup_runtime_artifacts(session, root, preserve_output=True)
