...
def cmd_ensure_stop_hook(args: list[str]) -> int:
    ...
    if not settings or not command:
        ...
        return 1
    provider = runtime_provider(Path(os.getcwd()))
    if provider != "claude":
        write_json({
            "ok": True,
            "changed": False,
            "reason": "skipped_non_claude_runtime",
            "provider": provider,
            "message": "Claude Stop hook setup skipped because the active runtime is not Claude.",
        })
        return 0
